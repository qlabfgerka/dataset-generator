...
def generate_cat_prompt() -> str:
    """Returns a random cat prompt in the format: "realistic photo [view] of [mood] [breed] cat [activity] [position]".
    ...
    return f"realistic photo {random.choice(views)} of {random.choice(moods)} {random.choice(breeds)} cat {random.choice(activities)} {random.choice(positions)}"
